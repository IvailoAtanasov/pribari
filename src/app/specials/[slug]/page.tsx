...
async function getSpecial(slug: string): Promise<DetailContent | null> {
  return client.fetch(
    specialDetailQuery,
    { slug },
    {
      next: { revalidate: 60, tags: ['special', `special-${slug}`] },
    }
  )
}

export default async function SpecialDetailPage({ params }: { params: Promise<{ slug: string | string[] }> }) {
  const resolved = await params
  const slugParam = Array.isArray(resolved.slug) ? resolved.slug[0] : resolved.slug
  if (!slugParam) return notFound()

  const product = await getSpecial(slugParam)
