...
const sweetDetailQuery = groq`
  *[_type == "sweet" && slug.current == $slug][0]{
    name,
    price,
    priceUnit,
    description,
...
async function getSweet(slug: string): Promise<DetailContent | null> {
  return client.fetch(
    sweetDetailQuery,
    { slug },
    {
      next: { revalidate: 60, tags: ['sweet', `sweet-${slug}`] },
    }
  )
}

export default async function SweetDetailPage({ params }: { params: Promise<{ slug: string | string[] }> }) {
  const resolved = await params
  const slugParam = Array.isArray(resolved.slug) ? resolved.slug[0] : resolved.slug
  if (!slugParam) return notFound()

  const product = await getSweet(slugParam)
